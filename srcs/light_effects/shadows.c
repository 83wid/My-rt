...
/*   Updated: 2021/04/26 14:09:57 by nabouzah         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				in_shadow(t_rt *rt, t_light *light, t_object *object, t_ray ray)
{
	t_object	*obj;
	t_object	o;
	double		t_d[2];
	...
	while (obj)
	{
				...
				&& (t_d[0] = rt->intersection[obj->type](&o, &shadow)) > 1e-5)
		{
			...
			if (t_d[0] > 0.0 && t_d[1] < light->d && !obj->is_transp && !obj->is_ref)
				return (0);
